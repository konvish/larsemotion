...
public:
    ...
    */
     ...
     * @param frame_start_pose      本分段第一个激光点对应的位姿 Ps
     * @param frame_end_pose        本分段最后一个激光点对应的位姿 Pe
     * @param ranges                激光数据－－距离
     * @param angles                激光数据－－角度
     * @param startIndex            本分段第一个激光点在激光帧中的下标 l
     * @param beam_number           本分段的激光点数量 N
     */
    ...
    {
       //TODO
       int delta_t = (frame_end_pose.stamp_.sec - frame_start_pose.stamp_.sec)/beam_number;
       tf::Vector3 delta_p = (frame_end_pose.getOrigin() - frame_start_pose.getOrigin())/beam_number;
       for (int i = 0;i < beam_number;i++){
           tf::Quaternion quat = frame_start_pose.getRotation();
           tf::Vector3 vec2 = frame_start_pose.getOrigin() + delta_p * i;
           tf::Transform p(quat,vec2);
           ros::Time i_t(frame_start_pose.stamp_.sec + delta_t * i);
           tf::Stamped<tf::Pose> frame_i_pose(p,i_t,frame_start_pose.frame_id_);
       }
       //end of TODO
    }
